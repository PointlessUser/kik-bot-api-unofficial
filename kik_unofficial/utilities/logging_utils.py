...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        return f'{time} {level_color}{level_name}:{reset} {level_icon} [Thread-{record.thread}:{record.threadName}] {message.replace(f"{level_name}: ", "")}'
        ...
        if log_file_path:
            ...
            if not os.path.exists(log_dir):
                try:
                    ...
                    self.logger.error(f"Could not create log directory: {str(e)}")
